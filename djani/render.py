...
def main():
    template = "<body><header>{{nesto}}</header> {{for users}}<h1 > {{first}} {{last}} < /h1 >{{end}}</body >"
    data = {'nesto': 'Prc', 'users':  [{'first': 'Nemanja', 'last': 'Mijailovic'},
                                       {'first': 'Bakir', 'last': 'Curic'}]}
    print(render(template, data))
    '''<body><header>Prc</header>
    <h1 > Nemanja Mijailovic < /h1 >
    <h1 > Bakir Curic < /h1 >
    body >'''
    template = "<h1>{{ first }} {{ last }}</h1>"
    data = {'first': ['x', 'y'], 'last': 'Curic'}
    print(render(template, data))

    # <h1>xy Curic</h1>

    template = "<body>{{for users}}<h1 > {{first}} {{last}} {{first}} < /h1 >{{end}}</body >"
    data = {'users':  [{'first': 'Nemanja', 'last': 'Mijailovic'},
                       {'first': 'Bakir', 'last': 'Curic'},
                       {'first': 'Mlilica', 'last': 'Miljkov'},
                       {'first': 'Amra', 'last': 'Curic'}]}
    print(render(template, data))

    '''
    <body>
    <h1 > Nemanja Mijailovic Nemanja < /h1 >
    <h1 > Bakir Curic Bakir < /h1 >
    <h1 > Mlilica Miljkov Mlilica < /h1 >
    <h1 > Amra Curic Amra < /h1 >
    body >'''
    # evo primera kad nije dobar for ili else

    '''
    template = "<body>{{fori users}}<h1 > {{first}} {{last}} < /h1 >{{end}}</body >"
    data = {'users':  [{'first': 'Nemanja', 'last': 'Mijailovic'},
                       {'first': 'Bakir', 'last': 'Curic'}]}
    print(render(template, data))
    '''
    ...
    Did not find key or 'for'''
    '''
    template = "<body>{{for users}}<h1 > {{first}} {{last}} < /h1 >{{ende}}</body >"
    data = {'users':  [{'first': 'Nemanja', 'last': 'Mijailovic'},
                       {'first': 'Bakir', 'last': 'Curic'}]}
    print(render(template, data))
    '''
